...
- **Транскрипт и диаризация** — новая вкладка: закинь аудио/видео или запиши с микрофона — получишь транскрипт с разбивкой по спикерам (NVIDIA Sortformer, до 4 голосов), прослушиванием сегментов и экспортом в `.txt`/`.srt`
- **Голоса из спикеров в один клик** — кнопка «Сделать голос» собирает из реплик спикера готовый референс с транскриптом и сохраняет персону в галерею — клонируй голос любого участника записи сразу
- **Очистка голоса** — вокал-сепарация референсов перед клонированием (Mel-Band Roformer, нативный BSRoformer.cpp на CUDA): музыка и фон убираются, клон цепляется за чистый голос; тумблер включён по умолчанию
