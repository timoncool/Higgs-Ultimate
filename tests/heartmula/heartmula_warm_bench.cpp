...
std::vector<std::pair<std::string, std::string>> parse_session_options(int argc, char ** argv) {
    ...
    for (int i = 1; i + 1 < argc; ++i) {
        ...
        if (eq == std::string::npos || eq == 0) {
            throw std::runtime_error("invalid HeartMuLa --session-option: " + option);
        }
...
engine::core::BackendType parse_backend(const std::string & value) {
    ...
    throw std::runtime_error("HeartMuLa warmbench is CUDA-only");
}
...
std::string required_string(const engine::io::json::Value & object, const std::string & key) {
    ...
    if (value.empty()) {
        throw std::runtime_error("HeartMuLa warmbench request missing field: " + key);
    }
    return value;
}
...
engine::runtime::TaskRequest make_request(const engine::io::json::Value & object) {
    engine::runtime::TaskRequest request;
    request.text_input = engine::runtime::Transcript{required_string(object, "lyrics"), ""};
    request.options["tags"] = required_string(object, "tags");
    set_optional_option(request, object, "max_audio_length_ms", "max_audio_length_ms");
    set_optional_option(request, object, "topk", "topk");
    set_optional_option(request, object, "temperature", "temperature");
    set_optional_option(request, object, "cfg_scale", "cfg_scale");
    set_optional_option(request, object, "codec_duration", "codec_duration");
    set_optional_option(request, object, "codec_steps", "codec_steps");
    set_optional_option(request, object, "codec_guidance_scale", "codec_guidance_scale");
...
std::vector<engine::runtime::TaskRequest> parse_requests(const std::string & request_sequence_json) {
    if (request_sequence_json.empty()) {
        throw std::runtime_error("HeartMuLa warmbench requires --request-sequence-json");
    }
    ...
    if (requests.empty()) {
        throw std::runtime_error("HeartMuLa warmbench request sequence is empty");
    }
    return requests;
}
...
engine::io::json::Value audio_summary_json(const engine::runtime::AudioBuffer & audio) {
    if (audio.samples.empty()) {
        throw std::runtime_error("HeartMuLa warmbench received empty audio output");
    }
    double sum = 0.0;
    double abs_sum = 0.0;
    double sq_sum = 0.0;
...
engine::io::json::Value step_json(
    ...
    if (!result.audio_output.has_value()) {
        throw std::runtime_error("HeartMuLa warmbench expected audio output");
    }
...
int main(int argc, char ** argv) {
    try {
        const std::filesystem::path model_path = arg_value(argc, argv, "--model", "models/HeartMuLa");
        ...
        const std::filesystem::path timing_path =
            arg_value(argc, argv, "--timing-file", "/tmp/heartmula_warm_bench_timing.log");
        ...
        load_request.family_hint = "heartmula";
        ...
        if (session == nullptr) {
            throw std::runtime_error("loaded HeartMuLa session is not offline-capable");
        }
        ...
        for (size_t request_index = 0; request_index < requests.size(); ++request_index) {
            ...
            std::cout << "heartmula.wall_ms=" << wall_ms << "\n";
        ...
        std::cerr << "heartmula_warm_bench failed: " << ex.what() << "\n";
        return 1;
    }
}
